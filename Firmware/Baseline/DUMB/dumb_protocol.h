#ifndef DUMB_PROTOCOL_H_
#define DUMB_PROTOCOL_H_

/*
 * DUMB protocol (v1.4-compatible) + compact inline utilities.
 * This header intentionally *replaces* the old dumb_helpers.h to
 * avoid duplication and ODR issues. Do not include dumb_helpers.h.
 * [802.15.4 MAC hdr]
 ...
 */
static inline uint8_t dumb_chk8(const uint8_t *buf, size_t len)
{
    uint16_t s = 0;
 ...
 * Creates:  none.
 */
static inline void dumb_u32_be_write(uint8_t *p, uint32_t v)
{
 ...
 * Creates:  none.
 */
static inline uint32_t dumb_u32_be_read(const uint8_t *p)
{
...
/* ------------------------------------------------------------------ */
/*  Random jitter (0..DUMB_JITTER_MS)                                  */
/* ------------------------------------------------------------------ */
/*
 * dumb_rand_jitter_ms()
 * What:     Produce 0..DUMB_JITTER_MS pseudo-random jitter in ms.
 * Methods:  Scale 16-bit random_rand() to [0..JITTER].
 * Creates:  none.
 */
static inline uint16_t dumb_rand_jitter_ms(void)
{
    unsigned int d_ms = 0;
    d_ms = (uint16_t)(((uint32_t)(random_rand() & 0xFFFFu) *
                       (uint32_t)DUMB_JITTER_MS) / 65535u);
 ...
 */
static inline uint8_t dumb_dir_from_delta(int32_t dx, int32_t dy)
{
    /*
     * dumb_dir_from_delta()
     * What:    Map dx,dy to octant 0..7; return DUMB_DIR_UNK when stationary.
    ...
    if (adx == 0 && ady == 0) return DUMB_DIR_UNK;

    if (adx >= (ady << 1)) return (dx >= 0) ? DUMB_DIR_E : DUMB_DIR_W;
    if (ady >= (adx << 1)) return (dy >= 0) ? DUMB_DIR_N : DUMB_DIR_S;

    if (dx >= 0 && dy >= 0) return DUMB_DIR_NE;
    if (dx <  0 && dy >= 0) return DUMB_DIR_NW;
    if (dx <  0 && dy <  0) return DUMB_DIR_SW;
    return DUMB_DIR_SE;
}
...
/* ------------------------------------------------------------------ */
#ifndef DUMB_DEDUP_MAX
#define DUMB_DEDUP_MAX 64
#endif
...
typedef struct {
    dumb_key_t items[DUMB_DEDUP_MAX];
    uint8_t    used;
 ...
 * Creates:  none.
 */
static inline void dumb_dedup_init(dumb_dedup_t *d)
{
    memset(d, 0, sizeof(*d));
}
 ...
 */
static inline uint8_t dumb_dedup_has(dumb_dedup_t *d,
                                     uint32_t origin,
                                     uint32_t msg)
{
    for(uint8_t i = 0; i < d->used; ++i) {
        uint8_t idx = (uint8_t)((i < DUMB_DEDUP_MAX) ? i
                                                    : (i % DUMB_DEDUP_MAX));
 ...
 * Creates:  none.
 */
static inline void dumb_dedup_put(dumb_dedup_t *d,
                                  uint32_t origin,
                                  uint32_t msg)
{
    d->items[d->head].origin_id = origin;
    d->items[d->head].msg_id    = msg;
    d->head = (uint8_t)((d->head + 1) % DUMB_DEDUP_MAX);
    if(d->used < DUMB_DEDUP_MAX) d->used++;
}
...
/*  The DUMB frame is rebuilt from this record at flush time.         */
/*                                                                    */
/*  Record layout (21 bytes):                                         */
/*    msg_type  (1)  — DUMB_MSG_DATA or DUMB_MSG_EMERGENCY            */
...
/*    dir8      (1)  — direction octant 0..7 or DUMB_DIR_UNK          */
...
/*   caused memory corruption when DUMB_CARRY_MAX_FRAME was 16)       */
/* ------------------------------------------------------------------ */
typedef struct {
    uint8_t  msg_type;      /* DUMB_MSG_DATA or DUMB_MSG_EMERGENCY */
    uint32_t origin_id;
    uint32_t msg_id;
    uint32_t ts_ms;
    int16_t  x_dm;
...
} __attribute__((packed)) dumb_carry_slot_t;

/* Compile-time sanity check (requires C11).
#if defined(__STDC_VERSION__) && (__STDC_VERSION__ >= 201112L)
_Static_assert(sizeof(dumb_carry_slot_t) == 23,
               "dumb_carry_slot_t size changed: check packing/layout");
#endif
*/

typedef struct {
    dumb_carry_slot_t slots[DUMB_CARRY_MAX_ITEMS];
    uint16_t          count;
 ...
 * Creates:  none.
 */
static inline void dumb_carry_init(dumb_carry_t *c)
{
    memset(c, 0, sizeof(*c));
}
 ...
 */
static inline uint8_t dumb_carry_exists(dumb_carry_t *c,
                                        uint32_t origin,
                                        uint32_t msg)
{
 ...
 * Methods:  Write fields at head; advance head mod DUMB_CARRY_MAX_ITEMS.
 * Creates:  none.
 */
static inline void dumb_carry_put(dumb_carry_t *c,
                                  uint8_t  msg_type,
                                  uint32_t origin,
                                  uint32_t msg,
                                  uint32_t ts_ms,
                                  ...
                                  uint8_t  ef_code)
{
    ...
    c->head = (uint16_t)((idx + 1u) % DUMB_CARRY_MAX_ITEMS);

    /* Count increases only when we consumed a previously free slot. */
    if(was_free && c->count < DUMB_CARRY_MAX_ITEMS) c->count++;
}
 ...
 * Creates:  loop index i.
 */
static inline void dumb_carry_ack(dumb_carry_t *c,
                                  uint32_t origin,
                                  uint32_t msg)
{
    for(uint16_t i = 0; i < DUMB_CARRY_MAX_ITEMS; ++i) {
 ...
 */
static inline uint32_t dumb_make_msgid(uint16_t *p_seq16)
{
...
/* ------------------------------------------------------------------ */
/*  DUMB wire format (pack/parse)                                      */
/* ------------------------------------------------------------------ */
/*
 * DUMB frame (v1.5) on the wire (BE multi-byte header fields):
 *   [ "DUMB"(4) ]
 ...
 *   [ CS(1) ]            // LSB of sum from 'D' to end of TimeStamp
 */
...
/* =========================================================================
 * Unified DUMB payload descriptions
 * ========================================================================= */
...
} __attribute__((packed)) dumb_data_payload_t;

/* EMERGENCY payload: EF code + same metrics */
typedef struct __attribute__((packed)) {
    char    sig[4];     /* "EFBR" */
    uint8_t ef_code;    /* DUMB_EF_* */
    int16_t x_dm;
    int16_t y_dm;
    uint16_t v_dmps;
    uint8_t dir8;
} dumb_emergency_payload_t;
...
dumb_pack(const dumb_fields_t *f, uint8_t *out, uint16_t cap, uint16_t *out_len)
{
    if(!f || !out || !out_len || cap < 32) return 0;

    /* "DUMB" */
    out[0] = 'D'; out[1] = 'U'; out[2] = 'M'; out[3] = 'B';
    ...
    /* SenderID BE */
    dumb_u32_be_write(&out[10], f->sender_id);

    /* MsgType */
    out[14] = f->msg_type;

    /* MsgID BE */
    dumb_u32_be_write(&out[15], f->msg_id);

    /* TargetID BE */
    dumb_u32_be_write(&out[19], f->target_id);

    /* OriginID BE */
    dumb_u32_be_write(&out[23], f->origin_id);

    /* Payload */
    if(f->payload_len > (DUMB_MAX_FRAME - 32)) return 0;
    if(f->payload_len && f->payload) {
        memcpy(&out[27], f->payload, f->payload_len);
    }

    /* TimeStamp BE */
    dumb_u32_be_write(&out[27 + f->payload_len], f->ts_ms);
    ...
    uint8_t cs = dumb_chk8(out, pre_cs_len);
...
/*
 * dumb_parse()
 * What:     Validate and parse DUMB frame from 'in'.
 * Methods:  Check signature/length/CS; expose payload view.
 */
static inline uint8_t
dumb_parse(const uint8_t *in, uint16_t in_len,
           dumb_fields_t *f, const uint8_t **pl, uint16_t *pl_len)
{
    if(!in || !f || !pl || !pl_len || in_len < 32) return 0;
    if(in[0] != 'D' || in[1] != 'U' || in[2] != 'M' || in[3] != 'B') return 0;

    uint16_t body_len = (uint16_t)(((uint16_t)in[5] << 8) | in[6]);
    uint16_t expect = (uint16_t)(4 + 3 + body_len);
    if(in_len != expect) return 0;

    uint8_t cs_calc = dumb_chk8(in, (uint16_t)(expect - 1));
    ...
    f->sender_id = dumb_u32_be_read(&in[10]);
    f->msg_type  = in[14];
    f->msg_id    = dumb_u32_be_read(&in[15]);
    f->target_id = dumb_u32_be_read(&in[19]);
    f->origin_id = dumb_u32_be_read(&in[23]);
    ...
    *pl_len = pay_len;
    ...
    f->ts_ms = dumb_u32_be_read(&in[27 + pay_len]);

    return 1;
}

#endif /* DUMB_PROTOCOL_H_ */
