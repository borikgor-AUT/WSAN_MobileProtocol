 ...
 * Methods:
 *   Decode payload fields (little-endian for metrics) and printf().
 * Creates:
                  ...
                  uint16_t pl_len)
{
    ...
    if(h->msg_type == BGKP_MSG_DATA) {
        ...
        if(pl != NULL && pl_len >= 7) {
            ...
            y = (int16_t)((uint16_t)pl[2] | ((uint16_t)pl[3] << 8));
            v = (uint16_t)((uint16_t)pl[4] | ((uint16_t)pl[5] << 8));
            dir = pl[6];
        }

        printf("RSU_RX type=DATA mode=RSU origin=%lu msg=%lu x=%ld y=%ld "
               "v=%u dir=%u ts=%lu\n",
    ...
    if(h->msg_type == BGKP_MSG_EMERGENCY) {
        ...
        if(pl != NULL && pl_len >= 5) {
            if(pl[0] == 'E' && pl[1] == 'F' && pl[2] == 'B' &&
               pl[3] == 'R') {
                code = pl[4];
                if(pl_len >= 12) {
                    x = (int16_t)((uint16_t)pl[5] |
                                  ((uint16_t)pl[6] << 8));
                    y = (int16_t)((uint16_t)pl[7] |
                                  ((uint16_t)pl[8] << 8));
                    v = (uint16_t)((uint16_t)pl[9] |
                                   ((uint16_t)pl[10] << 8));
                    dir = pl[11];
                }
            }
        ...
        printf("RSU_RX type=EF origin=%lu msg=%lu code=%u x=%ld y=%ld "
               "v=%u dir=%u ts=%lu\n",
               (unsigned long)h->origin_id,
               (unsigned long)h->msg_id,
               (unsigned)code,
